...
if submit_button:
    try:
        if not name:
            ...
            article_to_add = {'name': name.strip(), 'category': category, 'price': price, 'is_bought': False}
        ...
        for idx, article in category_articles.iterrows():
            if st.session_state.edit_idx == idx:
                ...
                with col1:
                    ...
                    st.write(text)
                with col2:
                    if st.button('✔️', key=f'set_is_bought_{article["name"]}_{idx}'):
                        set_is_bought(idx)
                        st.rerun()
                with col3:
