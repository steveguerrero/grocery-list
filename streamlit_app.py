...
if st.session_state.articles:
    ...
    for category in visible_categories:
        st.write(f"**{category}:**")
        category_df = df_articles[df_articles['category'] == category].copy()
        
        # Mark the 'name' columns with strikethrough if 'is_bought' is True
        category_df['name'] = category_df.apply(lambda row: strike_text(row['name']) if row['is_bought'] else row['name'], axis=1)
        
        st.dataframe(category_df.drop(columns=['is_bought']))
